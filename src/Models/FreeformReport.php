...
class FreeformReport extends AdvancedReport
{
    ...
    private static $allowed_types = array('Page' => 'Page', 'Member' => Member::class);
    ...
    protected function getReportableFields()
    {
        ...
        foreach ($tables as $type => $table) {
            ...
            $includeInheritedDbFields = false;
            $alias = '';
            $fieldPrefix = $type . '.';
            if (strpos($type, '.')) {
     ...
     */
    protected function applyRelation($modelClass, SQLSelect $query, $relation)
    {
        // NO-OP
        if (!$relation) return;
        ...
        foreach ($relation as $rel) {
            ...
            if ($component = $model->has_one($rel)) {
                if (!$query->isJoinedTo($alias)) {
                    ...
                    $realModelClass = DataObject::getSchema()->tableForField($modelClass, "{$foreignKey}ID");

                    $query->addLeftJoin(
                        $component,
                        "\"$alias\".\"ID\" = \"{$realModelClass}\".\"{$foreignKey}ID\"",
                        $alias
                    );
                ...
                $parentBaseClass = DataObject::getSchema()->baseDataClass($parentClass);
                $componentBaseClass = DataObject::getSchema()->baseDataClass($componentClass);
                $query->addInnerJoin(
                    $relationTable,
