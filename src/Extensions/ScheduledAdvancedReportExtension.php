...
namespace SilverstripeAustralia\AdvancedReports\Extensions;

use SilverstripeAustralia\AdvancedReports\Jobs\ScheduledReportJob;
...
class ScheduledAdvancedReportExtension extends DataExtension
{
    ...
    {
        Requirements::javascript('silverstripe-australia/advancedreports:javascript/scheduled-report-settings.js');
        ...
        if ($this->owner->Scheduled) {
            ...
            if (!$jobExists) {
                ...
                $this->owner->QueuedJobID = singleton('QueuedJobService')->queueJob(
                    new ScheduledReportJob($this->owner),
                    $time
                );
            }
        } else {
